...
/**
 * Data Mapper implementation for Atezate\Model\TurnosRelCamiones
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
class TurnosRelCamiones extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\TurnosRelCamiones';
     ...
     *
     * @param Atezate\Model\Raw\TurnosRelCamiones $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\TurnosRelCamiones) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\TurnosRelCamiones object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\TurnosRelCamiones object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            'id' => $model->getId(),
            'turnoId' => $model->getTurnoId(),
            'camionesId' => $model->getCamionesId(),
            'tabletId' => $model->getTabletId(),
            'orden' => $model->getOrden(),
            'origenPuntoRecogidaId' => $model->getOrigenPuntoRecogidaId(),
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\TurnosRelCamiones
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\TurnosRelCamiones');
        }
     ...
     *
     * @param Atezate\Model\Raw\TurnosRelCamiones $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\TurnosRelCamiones) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\TurnosRelCamiones object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\TurnosRelCamiones object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\TurnosRelCamiones', $capitalizedFk);
     ...
     */
    public function save(\Atezate\Model\Raw\TurnosRelCamiones $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\TurnosRelCamiones $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Raw\TurnosRelCamiones $model,
            ...
            if ($recursive) {
                if ($model->getParadas(null, null, true) !== null) {
                    ...
                    foreach ($paradas as $value) {
                        $value->setTurnosCamionesId($primaryKey)
                              ->saveRecursive(false, $transactionTag);
                    }
                }

                if ($model->getPosicionamiento(null, null, true) !== null) {
                    $posicionamiento = $model->getPosicionamiento();

                    if (!is_array($posicionamiento)) {

                        $posicionamiento = array($posicionamiento);
                    }

                    foreach ($posicionamiento as $value) {
                        $value->setTurnosCamionesId($primaryKey)
                              ->saveRecursive(false, $transactionTag);
                    }
                }

                if ($model->getTurnosCamionesRelOperarios(null, null, true) !== null) {
                    $turnosCamionesRelOperarios = $model->getTurnosCamionesRelOperarios();

                    if (!is_array($turnosCamionesRelOperarios)) {

                        $turnosCamionesRelOperarios = array($turnosCamionesRelOperarios);
                    }

                    foreach ($turnosCamionesRelOperarios as $value) {
                        $value->setTurnosRelCamionesId($primaryKey)
                              ->saveRecursive(false, $transactionTag);
                    }
                }
     ...
     * @param Atezate\Model\Raw\TurnosRelCamiones|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\TurnosRelCamiones The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\TurnosRelCamiones();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ...
                  ->setCamionesId($data['camionesId'])
                  ->setTabletId($data['tabletId'])
                  ->setOrden($data['orden'])
                  ->setOrigenPuntoRecogidaId($data['origenPuntoRecogidaId'])
        ...
        } else if ($data instanceof \Atezate\Model\Raw\TurnosRelCamiones) {
            $entry->setId($data->getId())
                  ->setTurnoId($data->getTurnoId())
                  ->setCamionesId($data->getCamionesId())
                  ->setTabletId($data->getTabletId())
                  ->setOrden($data->getOrden())
                  ->setOrigenPuntoRecogidaId($data->getOrigenPuntoRecogidaId())
                  ->setHoraInicio($data->getHoraInicio());
