...
/**
 * Data Mapper implementation for Atezate\Model\RutasRelPuntosRecogida
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
class RutasRelPuntosRecogida extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\RutasRelPuntosRecogida';
     ...
     *
     * @param Atezate\Model\Raw\RutasRelPuntosRecogida $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\RutasRelPuntosRecogida) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\RutasRelPuntosRecogida object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\RutasRelPuntosRecogida object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\RutasRelPuntosRecogida
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\RutasRelPuntosRecogida');
        }
     ...
     *
     * @param Atezate\Model\Raw\RutasRelPuntosRecogida $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\RutasRelPuntosRecogida) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\RutasRelPuntosRecogida object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\RutasRelPuntosRecogida object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\RutasRelPuntosRecogida', $capitalizedFk);
     ...
     */
    public function save(\Atezate\Model\Raw\RutasRelPuntosRecogida $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\RutasRelPuntosRecogida $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Raw\RutasRelPuntosRecogida $model,
     ...
     * @param Atezate\Model\Raw\RutasRelPuntosRecogida|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\RutasRelPuntosRecogida The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\RutasRelPuntosRecogida();
        }
        ...
        } else if ($data instanceof \Atezate\Model\Raw\RutasRelPuntosRecogida) {
