...
/**
 * Data Mapper implementation for Atezate\Model\CubosTipos
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
class CubosTipos extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\CubosTipos';
     ...
     *
     * @param Atezate\Model\Raw\CubosTipos $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\CubosTipos) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\CubosTipos object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\CubosTipos object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            ...
            'tipo' => $model->getTipo(),
            'litros' => $model->getLitros(),
            'color' => $model->getColor(),
            'altura' => $model->getAltura(),
            'anchura' => $model->getAnchura(),
            'longitud' => $model->getLongitud(),
            'llave' => $model->getLlave(),
            'llenadoObligatorio' => $model->getLlenadoObligatorio(),
            'marcaCuboId' => $model->getMarcaCuboId(),
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\CubosTipos
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\CubosTipos');
        }
     ...
     *
     * @param Atezate\Model\Raw\CubosTipos $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\CubosTipos) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\CubosTipos object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\CubosTipos object in delete for " . get_class($this);
            }
        ...
        try {
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\CubosTipos', $capitalizedFk);
            ...
            $where = $dbAdapter->quoteInto($dbAdapter->quoteIdentifier('cubosTiposId') . ' = ?', $model->getCubosTiposId());
     ...
     */
    public function save(\Atezate\Model\Raw\CubosTipos $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\CubosTipos $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Raw\CubosTipos $model,
    ...
    )
    {
        ...
        $primaryKey = $model->getCubosTiposId();
        $success = true;
        ...
        try {
            if (is_null($primaryKey) || empty($primaryKey)) {
                ...
                if ($primaryKey) {
                    $model->setCubosTiposId($primaryKey);
                } else {
                ...
                $this->getDbTable()
                     ->update(
                         $data,
                         array(
                             $this->getDbTable()->getAdapter()->quoteIdentifier('cubosTiposId') . ' = ?' => $primaryKey
                         )
                     );
            }
            ...
            if ($recursive) {
                if ($model->getCubos(null, null, true) !== null) {
                    ...
                    foreach ($cubos as $value) {
                        $value->setCubosTiposId($primaryKey)
     ...
     * @param Atezate\Model\Raw\CubosTipos|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\CubosTipos The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\CubosTipos();
        }
        ...
        if (is_array($data)) {
            $entry->setCubosTiposId($data['cubosTiposId'])
                  ->setTipo($data['tipo'])
                  ->setLitros($data['litros'])
                  ->setColor($data['color'])
                  ->setAltura($data['altura'])
                  ->setAnchura($data['anchura'])
                  ->setLongitud($data['longitud'])
                  ->setLlave($data['llave'])
                  ->setLlenadoObligatorio($data['llenadoObligatorio'])
                  ->setMarcaCuboId($data['marcaCuboId'])
        ...
        } else if ($data instanceof \Atezate\Model\Raw\CubosTipos) {
            $entry->setCubosTiposId($data->getCubosTiposId())
                  ->setTipo($data->getTipo())
                  ->setLitros($data->getLitros())
                  ->setColor($data->getColor())
                  ->setAltura($data->getAltura())
                  ->setAnchura($data->getAnchura())
                  ->setLongitud($data->getLongitud())
                  ->setLlave($data->getLlave())
                  ->setLlenadoObligatorio($data->getLlenadoObligatorio())
                  ->setMarcaCuboId($data->getMarcaCuboId())
                  ->setFabricante($data->getFabricante());
