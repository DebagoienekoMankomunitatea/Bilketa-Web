...
/**
 * Data Mapper implementation for Atezate\Model\RecogidaTipos
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
class RecogidaTipos extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\RecogidaTipos';
     ...
     *
     * @param Atezate\Model\Raw\RecogidaTipos $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\RecogidaTipos) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\RecogidaTipos object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\RecogidaTipos object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            'recogidaTiposId' => $model->getRecogidaTiposId(),
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\RecogidaTipos
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\RecogidaTipos');
        }
     ...
     *
     * @param Atezate\Model\Raw\RecogidaTipos $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\RecogidaTipos) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\RecogidaTipos object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\RecogidaTipos object in delete for " . get_class($this);
            }
        ...
        try {
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\RecogidaTipos', $capitalizedFk);
            ...
            $where = $dbAdapter->quoteInto($dbAdapter->quoteIdentifier('recogidaTiposId') . ' = ?', $model->getRecogidaTiposId());
     ...
     */
    public function save(\Atezate\Model\Raw\RecogidaTipos $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\RecogidaTipos $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Raw\RecogidaTipos $model,
    ...
    )
    {
        ...
        $primaryKey = $model->getRecogidaTiposId();
        $success = true;
        ...
        try {
            if (is_null($primaryKey) || empty($primaryKey)) {
                ...
                if ($primaryKey) {
                    $model->setRecogidaTiposId($primaryKey);
                } else {
                ...
                $this->getDbTable()
                     ->update(
                         $data,
                         array(
                             $this->getDbTable()->getAdapter()->quoteIdentifier('recogidaTiposId') . ' = ?' => $primaryKey
                         )
                     );
            }
            ...
            if ($recursive) {
                if ($model->getRecogidas(null, null, true) !== null) {
                    ...
                    foreach ($recogidas as $value) {
                        $value->setRecogidaTipoId($primaryKey)
     ...
     * @param Atezate\Model\Raw\RecogidaTipos|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\RecogidaTipos The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\RecogidaTipos();
        }
        ...
        if (is_array($data)) {
            $entry->setRecogidaTiposId($data['recogidaTiposId'])
        ...
        } else if ($data instanceof \Atezate\Model\Raw\RecogidaTipos) {
            $entry->setRecogidaTiposId($data->getRecogidaTiposId())
