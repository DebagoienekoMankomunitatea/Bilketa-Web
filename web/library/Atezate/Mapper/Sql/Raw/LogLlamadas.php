...
/**
 * Data Mapper implementation for Atezate\Model\LogLlamadas
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
class LogLlamadas extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\LogLlamadas';
     ...
     *
     * @param Atezate\Model\Raw\LogLlamadas $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\LogLlamadas) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\LogLlamadas object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\LogLlamadas object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            'llamadasId' => $model->getLlamadasId(),
            'incidenciaId' => $model->getIncidenciaId(),
            'dia' => $model->getDia(),
            'hora' => $model->getHora(),
            'contactado' => $model->getContactado(),
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\LogLlamadas
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\LogLlamadas');
        }
     ...
     *
     * @param Atezate\Model\Raw\LogLlamadas $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\LogLlamadas) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\LogLlamadas object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\LogLlamadas object in delete for " . get_class($this);
            }
        ...
        try {
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\LogLlamadas', $capitalizedFk);
            ...
            $where = $dbAdapter->quoteInto($dbAdapter->quoteIdentifier('llamadasId') . ' = ?', $model->getLlamadasId());
     ...
     */
    public function save(\Atezate\Model\Raw\LogLlamadas $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\LogLlamadas $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Raw\LogLlamadas $model,
    ...
    )
    {
        ...
        $primaryKey = $model->getLlamadasId();
        $success = true;
        ...
        try {
            if (is_null($primaryKey) || empty($primaryKey)) {
                ...
                if ($primaryKey) {
                    $model->setLlamadasId($primaryKey);
                } else {
                ...
                $this->getDbTable()
                     ->update(
                         $data,
                         array(
                             $this->getDbTable()->getAdapter()->quoteIdentifier('llamadasId') . ' = ?' => $primaryKey
                         )
                     );
            }
     ...
     * @param Atezate\Model\Raw\LogLlamadas|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\LogLlamadas The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\LogLlamadas();
        }
        ...
        if (is_array($data)) {
            $entry->setLlamadasId($data['llamadasId'])
                  ->setIncidenciaId($data['incidenciaId'])
                  ->setDia($data['dia'])
                  ->setHora($data['hora'])
                  ->setContactado($data['contactado'])
        ...
        } else if ($data instanceof \Atezate\Model\Raw\LogLlamadas) {
            $entry->setLlamadasId($data->getLlamadasId())
                  ->setIncidenciaId($data->getIncidenciaId())
                  ->setDia($data->getDia())
                  ->setHora($data->getHora())
                  ->setContactado($data->getContactado())
