...
/**
 * Data Mapper implementation for Atezate\Model\MarcasCubo
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
class MarcasCubo extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\MarcasCubo';
     ...
     *
     * @param Atezate\Model\Raw\MarcasCubo $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\MarcasCubo) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\MarcasCubo object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\MarcasCubo object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\MarcasCubo
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\MarcasCubo');
        }
     ...
     *
     * @param Atezate\Model\Raw\MarcasCubo $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\MarcasCubo) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\MarcasCubo object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\MarcasCubo object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\MarcasCubo', $capitalizedFk);
     ...
     */
    public function save(\Atezate\Model\Raw\MarcasCubo $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\MarcasCubo $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Raw\MarcasCubo $model,
    ...
    )
    {
        ...
        try {
            ...
            if ($recursive) {
                if ($model->getCubosTipos(null, null, true) !== null) {
                    $cubosTipos = $model->getCubosTipos();

                    if (!is_array($cubosTipos)) {

                        $cubosTipos = array($cubosTipos);
                    }

                    foreach ($cubosTipos as $value) {
                        $value->setMarcaCuboId($primaryKey)
     ...
     * @param Atezate\Model\Raw\MarcasCubo|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\MarcasCubo The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\MarcasCubo();
        }
        ...
        } else if ($data instanceof \Atezate\Model\Raw\MarcasCubo) {
