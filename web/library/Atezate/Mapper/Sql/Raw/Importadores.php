...
class Importadores extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\Importadores';
     ...
     *
     * @param Atezate\Model\Raw\Importadores $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\Importadores) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\Importadores object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\Importadores object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            'id' => $model->getId(),
            'archivoFileSize' => $model->getArchivoFileSize(),
            'archivoMimeType' => $model->getArchivoMimeType(),
            'archivoBaseName' => $model->getArchivoBaseName(),
            'tipo' => $model->getTipo(),
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\Importadores
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\Importadores');
        }
     ...
     *
     * @param Atezate\Model\Raw\Importadores $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\Importadores) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\Importadores object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\Importadores object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\Importadores', $capitalizedFk);
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\Importadores $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Raw\Importadores $model,
     ...
     * @param Atezate\Model\Raw\Importadores|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\Importadores The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\Importadores();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ->setArchivoFileSize($data['archivoFileSize'])
                  ->setArchivoMimeType($data['archivoMimeType'])
                  ->setArchivoBaseName($data['archivoBaseName'])
                  ->setTipo($data['tipo'])
        ...
        } else if ($data instanceof \Atezate\Model\Raw\Importadores) {
            $entry->setId($data->getId())
                  ->setArchivoFileSize($data->getArchivoFileSize())
                  ->setArchivoMimeType($data->getArchivoMimeType())
                  ->setArchivoBaseName($data->getArchivoBaseName())
