...
class Paradas extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\Paradas';
     ...
     *
     * @param Atezate\Model\Raw\Paradas $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\Paradas) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\Paradas object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\Paradas object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            ...
            'horaFinal' => $model->getHoraFinal(),
            'puntosRecogidasId' => $model->getPuntosRecogidasId(),
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\Paradas
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\Paradas');
        }
     ...
     *
     * @param Atezate\Model\Raw\Paradas $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\Paradas) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\Paradas object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\Paradas object in delete for " . get_class($this);
            }
        ...
        try {
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\Paradas', $capitalizedFk);
            ...
            $where = $dbAdapter->quoteInto($dbAdapter->quoteIdentifier('paradaId') . ' = ?', $model->getParadaId());
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\Paradas $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Raw\Paradas $model,
            ...
            if (is_null($primaryKey) || empty($primaryKey)) {
                ...
                $this->getDbTable()
                     ->update(
                         $data,
                         array(
                             $this->getDbTable()->getAdapter()->quoteIdentifier('paradaId') . ' = ?' => $primaryKey
                         )
                     );
            }
     ...
     * @param Atezate\Model\Raw\Paradas|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\Paradas The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\Paradas();
        }
        ...
        } else if ($data instanceof \Atezate\Model\Raw\Paradas) {
            $entry->setParadaId($data->getParadaId())
                  ->setTurnosCamionesId($data->getTurnosCamionesId())
                  ->setHoraInicio($data->getHoraInicio())
                  ->setFinalizado($data->getFinalizado())
                  ->setHoraFinal($data->getHoraFinal())
                  ->setPuntosRecogidasId($data->getPuntosRecogidasId())
