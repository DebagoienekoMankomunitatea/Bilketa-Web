...
/**
 * Data Mapper implementation for Atezate\Model\KlearUsersRoles
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
{
    protected $_modelName = 'Atezate\\Model\\KlearUsersRoles';
     ...
     *
     * @param Atezate\Model\Raw\KlearUsersRoles $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\KlearUsersRoles) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\KlearUsersRoles object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\KlearUsersRoles object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\KlearUsersRoles
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\KlearUsersRoles');
        }
     ...
     *
     * @param Atezate\Model\Raw\KlearUsersRoles $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\KlearUsersRoles) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\KlearUsersRoles object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\KlearUsersRoles object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\KlearUsersRoles', $capitalizedFk);
     ...
     */
    public function save(\Atezate\Model\Raw\KlearUsersRoles $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\KlearUsersRoles $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Raw\KlearUsersRoles $model,
     ...
     * @param Atezate\Model\Raw\KlearUsersRoles|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\KlearUsersRoles The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\KlearUsersRoles();
        }
        ...
        } else if ($data instanceof \Atezate\Model\Raw\KlearUsersRoles) {
