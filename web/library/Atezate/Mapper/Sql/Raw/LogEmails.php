...
/**
 * Data Mapper implementation for Atezate\Model\LogEmails
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
class LogEmails extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\LogEmails';
     ...
     *
     * @param Atezate\Model\Raw\LogEmails $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\LogEmails) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\LogEmails object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\LogEmails object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            'emailMensajeId' => $model->getEmailMensajeId(),
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\LogEmails
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\LogEmails');
        }
     ...
     *
     * @param Atezate\Model\Raw\LogEmails $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\LogEmails) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\LogEmails object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\LogEmails object in delete for " . get_class($this);
            }
        ...
        try {
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\LogEmails', $capitalizedFk);
            ...
            $where = $dbAdapter->quoteInto($dbAdapter->quoteIdentifier('emailMensajeId') . ' = ?', $model->getEmailMensajeId());
     ...
     */
    public function save(\Atezate\Model\Raw\LogEmails $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\LogEmails $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Raw\LogEmails $model,
    ...
    )
    {
        ...
        $primaryKey = $model->getEmailMensajeId();
        $success = true;
        ...
        try {
            if (is_null($primaryKey) || empty($primaryKey)) {
                ...
                if ($primaryKey) {
                    $model->setEmailMensajeId($primaryKey);
                } else {
                ...
                $this->getDbTable()
                     ->update(
                         $data,
                         array(
                             $this->getDbTable()->getAdapter()->quoteIdentifier('emailMensajeId') . ' = ?' => $primaryKey
                         )
                     );
            }
     ...
     * @param Atezate\Model\Raw\LogEmails|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\LogEmails The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\LogEmails();
        }
        ...
        if (is_array($data)) {
            $entry->setEmailMensajeId($data['emailMensajeId'])
        ...
        } else if ($data instanceof \Atezate\Model\Raw\LogEmails) {
            $entry->setEmailMensajeId($data->getEmailMensajeId())
