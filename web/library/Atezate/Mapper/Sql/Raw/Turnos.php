...
class Turnos extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\Turnos';
     ...
     *
     * @param Atezate\Model\Raw\Turnos $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\Turnos) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\Turnos object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\Turnos object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\Turnos
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\Turnos');
        }
     ...
     *
     * @param Atezate\Model\Raw\Turnos $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\Turnos) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\Turnos object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\Turnos object in delete for " . get_class($this);
            }
        ...
        try {
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\Turnos', $capitalizedFk);
            ...
            $where = $dbAdapter->quoteInto($dbAdapter->quoteIdentifier('turnoId') . ' = ?', $model->getTurnoId());
     ...
     */
    public function save(\Atezate\Model\Raw\Turnos $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\Turnos $model, $useTransaction = true, $transactionTag = null)
    {
            ...
            if (is_null($primaryKey) || empty($primaryKey)) {
                ...
                $this->getDbTable()
                     ->update(
                         $data,
                         array(
                             $this->getDbTable()->getAdapter()->quoteIdentifier('turnoId') . ' = ?' => $primaryKey
                         )
                     );
            }
     ...
     * @param Atezate\Model\Raw\Turnos|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\Turnos The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\Turnos();
        }
        ...
        } else if ($data instanceof \Atezate\Model\Raw\Turnos) {
            $entry->setTurnoId($data->getTurnoId())
                  ->setRutaId($data->getRutaId())
                  ->setFecha($data->getFecha())
                  ->setHoraInicio($data->getHoraInicio())
                  ->setHoraFinal($data->getHoraFinal())
                  ->setCoste($data->getCoste());
