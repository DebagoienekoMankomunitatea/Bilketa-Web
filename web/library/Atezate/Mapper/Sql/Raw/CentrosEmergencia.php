...
/**
 * Data Mapper implementation for Atezate\Model\CentrosEmergencia
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
class CentrosEmergencia extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\CentrosEmergencia';
     ...
     *
     * @param Atezate\Model\Raw\CentrosEmergencia $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\CentrosEmergencia) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\CentrosEmergencia object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\CentrosEmergencia object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\CentrosEmergencia
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\CentrosEmergencia');
        }
     ...
     *
     * @param Atezate\Model\Raw\CentrosEmergencia $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\CentrosEmergencia) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\CentrosEmergencia object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\CentrosEmergencia object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\CentrosEmergencia', $capitalizedFk);
     ...
     */
    public function save(\Atezate\Model\Raw\CentrosEmergencia $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\CentrosEmergencia $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Raw\CentrosEmergencia $model,
            ...
            if ($recursive) {
                if ($model->getCodigosApertura(null, null, true) !== null) {
                    ...
                    foreach ($codigosApertura as $value) {
                        $value->setCentroEmergenciaId($primaryKey)
                ...
                if ($model->getCubos(null, null, true) !== null) {
                    ...
                    foreach ($cubos as $value) {
                        $value->setCentrosEmergenciaId($primaryKey)
                              ->saveRecursive(false, $transactionTag);
                    }
                }
     ...
     * @param Atezate\Model\Raw\CentrosEmergencia|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\CentrosEmergencia The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\CentrosEmergencia();
        }
        ...
        } else if ($data instanceof \Atezate\Model\Raw\CentrosEmergencia) {
            $entry->setId($data->getId())
                  ->setIp($data->getIp())
                  ->setPuerto($data->getPuerto())
                  ->setPuntoRecogidaId($data->getPuntoRecogidaId())
                  ->setIdentificador($data->getIdentificador());
