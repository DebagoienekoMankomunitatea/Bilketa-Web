...
/**
 * Data Mapper implementation for Atezate\Model\MunicipiosRelCercania
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
class MunicipiosRelCercania extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\MunicipiosRelCercania';
     ...
     *
     * @param Atezate\Model\Raw\MunicipiosRelCercania $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\MunicipiosRelCercania) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\MunicipiosRelCercania object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\MunicipiosRelCercania object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            ...
            'municipioCercanoId' => $model->getMunicipioCercanoId(),
        );
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\MunicipiosRelCercania
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\MunicipiosRelCercania');
        }
     ...
     *
     * @param Atezate\Model\Raw\MunicipiosRelCercania $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\MunicipiosRelCercania) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\MunicipiosRelCercania object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\MunicipiosRelCercania object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\MunicipiosRelCercania', $capitalizedFk);
     ...
     */
    public function save(\Atezate\Model\Raw\MunicipiosRelCercania $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\MunicipiosRelCercania $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Raw\MunicipiosRelCercania $model,
     ...
     * @param Atezate\Model\Raw\MunicipiosRelCercania|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\MunicipiosRelCercania The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\MunicipiosRelCercania();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ...
                  ->setMunicipioCercanoId($data['municipioCercanoId']);
        } else if ($data instanceof \Zend_Db_Table_Row_Abstract || $data instanceof \stdClass) {
            $entry->setId($data->{'id'})
                  ->setMunicipioId($data->{'municipioId'})
                  ->setMunicipioCercanoId($data->{'municipioCercanoId'});

        } else if ($data instanceof \Atezate\Model\Raw\MunicipiosRelCercania) {
            $entry->setId($data->getId())
                  ->setMunicipioId($data->getMunicipioId())
                  ->setMunicipioCercanoId($data->getMunicipioCercanoId());
