...
class Compostadores extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\Compostadores';
     ...
     *
     * @param Atezate\Model\Raw\Compostadores $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\Compostadores) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\Compostadores object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\Compostadores object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            'compostadoresId' => $model->getCompostadoresId(),
            'altaFecha' => $model->getAltaFecha(),
            'emplazamiento' => $model->getEmplazamiento(),
            'marcaCompostadorId' => $model->getMarcaCompostadorId(),
            'ventajasComunidad' => $model->getVentajasComunidad(),
            'privado' => $model->getPrivado(),
            'compostadoresIden' => $model->getCompostadoresIden(),
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\Compostadores
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\Compostadores');
        }
     ...
     *
     * @param Atezate\Model\Raw\Compostadores $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\Compostadores) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\Compostadores object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\Compostadores object in delete for " . get_class($this);
            }
        ...
        try {
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\Compostadores', $capitalizedFk);
            ...
            $where = $dbAdapter->quoteInto($dbAdapter->quoteIdentifier('compostadoresId') . ' = ?', $model->getCompostadoresId());
     ...
     */
    public function save(\Atezate\Model\Raw\Compostadores $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\Compostadores $model, $useTransaction = true, $transactionTag = null)
    {
    ...
    )
    {
        ...
        $primaryKey = $model->getCompostadoresId();
        $success = true;
        ...
        try {
            if (is_null($primaryKey) || empty($primaryKey)) {
                ...
                if ($primaryKey) {
                    $model->setCompostadoresId($primaryKey);
                } else {
                ...
                $this->getDbTable()
                     ->update(
                         $data,
                         array(
                             $this->getDbTable()->getAdapter()->quoteIdentifier('compostadoresId') . ' = ?' => $primaryKey
                         )
                     );
            }
            ...
            if ($recursive) {
                if ($model->getCompostadoresRelContribuyentes(null, null, true) !== null) {
                    ...
                    foreach ($compostadoresRelContribuyentes as $value) {
                        $value->setCompostadoresId($primaryKey)
                ...
                if ($model->getIncidencias(null, null, true) !== null) {
                    ...
                    foreach ($incidencias as $value) {
                        $value->setCompostadorId($primaryKey)
                              ->saveRecursive(false, $transactionTag);
                    }
                }

                if ($model->getRevision(null, null, true) !== null) {
     ...
     * @param Atezate\Model\Raw\Compostadores|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\Compostadores The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\Compostadores();
        }
        ...
        if (is_array($data)) {
            $entry->setCompostadoresId($data['compostadoresId'])
                  ->setAltaFecha($data['altaFecha'])
                  ->setEmplazamiento($data['emplazamiento'])
                  ->setMarcaCompostadorId($data['marcaCompostadorId'])
                  ->setVentajasComunidad($data['ventajasComunidad'])
                  ->setPrivado($data['privado'])
                  ->setCompostadoresIden($data['compostadoresIden'])
        ...
        } else if ($data instanceof \Atezate\Model\Raw\Compostadores) {
            $entry->setCompostadoresId($data->getCompostadoresId())
                  ->setAltaFecha($data->getAltaFecha())
                  ->setEmplazamiento($data->getEmplazamiento())
                  ->setMarcaCompostadorId($data->getMarcaCompostadorId())
                  ->setVentajasComunidad($data->getVentajasComunidad())
                  ->setPrivado($data->getPrivado())
                  ->setCompostadoresIden($data->getCompostadoresIden())
