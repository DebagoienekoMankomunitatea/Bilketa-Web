...
/**
 * Data Mapper implementation for Atezate\Model\ResiduosTipos
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
class ResiduosTipos extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\ResiduosTipos';
     ...
     *
     * @param Atezate\Model\Raw\ResiduosTipos $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\ResiduosTipos) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\ResiduosTipos object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\ResiduosTipos object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\ResiduosTipos
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\ResiduosTipos');
        }
     ...
     *
     * @param Atezate\Model\Raw\ResiduosTipos $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\ResiduosTipos) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\ResiduosTipos object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\ResiduosTipos object in delete for " . get_class($this);
            }
        ...
        try {
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\ResiduosTipos', $capitalizedFk);
            ...
            $where = $dbAdapter->quoteInto($dbAdapter->quoteIdentifier('residuosTiposId') . ' = ?', $model->getResiduosTiposId());
     ...
     */
    public function save(\Atezate\Model\Raw\ResiduosTipos $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\ResiduosTipos $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Raw\ResiduosTipos $model,
    ...
    )
    {
        ...
        $primaryKey = $model->getResiduosTiposId();
        $success = true;
        ...
        try {
            if (is_null($primaryKey) || empty($primaryKey)) {
                ...
                if ($primaryKey) {
                    $model->setResiduosTiposId($primaryKey);
                } else {
                ...
                $this->getDbTable()
                     ->update(
                         $data,
                         array(
                             $this->getDbTable()->getAdapter()->quoteIdentifier('residuosTiposId') . ' = ?' => $primaryKey
                         )
                     );
            }
            ...
            if ($recursive) {
                if ($model->getPuntosDescarga(null, null, true) !== null) {
                    $puntosDescarga = $model->getPuntosDescarga();

                    if (!is_array($puntosDescarga)) {

                        $puntosDescarga = array($puntosDescarga);
                    }

                    foreach ($puntosDescarga as $value) {
                        $value->setResiduosTiposId($primaryKey)
     ...
     * @param Atezate\Model\Raw\ResiduosTipos|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\ResiduosTipos The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\ResiduosTipos();
        }
        ...
        if (is_array($data)) {
            $entry->setResiduosTiposId($data['residuosTiposId'])
        ...
        } else if ($data instanceof \Atezate\Model\Raw\ResiduosTipos) {
            $entry->setResiduosTiposId($data->getResiduosTiposId())
