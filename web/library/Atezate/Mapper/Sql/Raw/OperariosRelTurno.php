...
/**
 * Data Mapper implementation for Atezate\Model\OperariosRelTurno
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
class OperariosRelTurno extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\OperariosRelTurno';
     ...
     *
     * @param Atezate\Model\OperariosRelTurno $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\OperariosRelTurno) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\OperariosRelTurno object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\OperariosRelTurno object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\OperariosRelTurno
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\OperariosRelTurno');
        }
     ...
     *
     * @param Atezate\Model\OperariosRelTurno $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\OperariosRelTurno) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\OperariosRelTurno object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\OperariosRelTurno object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\OperariosRelTurno', $capitalizedFk);
     ...
     */
    public function save(\Atezate\Model\OperariosRelTurno $model)
    {
     ...
     * @return boolean If the save action was successful
     */
    public function saveRecursive(\Atezate\Model\OperariosRelTurno $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\OperariosRelTurno $model,
     ...
     * @param Atezate\Model\OperariosRelTurno|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\OperariosRelTurno The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\OperariosRelTurno();
        }
        ...
        } else if ($data instanceof \Atezate\Model\OperariosRelTurno) {
