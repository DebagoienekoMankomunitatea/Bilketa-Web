...
/**
 * Data Mapper implementation for Atezate\Model\Dispositivos
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
class Dispositivos extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\Dispositivos';
     ...
     *
     * @param Atezate\Model\Raw\Dispositivos $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\Dispositivos) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\Dispositivos object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\Dispositivos object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            'id' => $model->getId(),
            'marcaDispositivoId' => $model->getMarcaDispositivoId(),
            'imei' => $model->getImei(),
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\Dispositivos
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\Dispositivos');
        }
     ...
     *
     * @param Atezate\Model\Raw\Dispositivos $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\Dispositivos) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\Dispositivos object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\Dispositivos object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\Dispositivos', $capitalizedFk);
     ...
     */
    public function save(\Atezate\Model\Raw\Dispositivos $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\Dispositivos $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Raw\Dispositivos $model,
     ...
     * @param Atezate\Model\Raw\Dispositivos|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\Dispositivos The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\Dispositivos();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ->setMarcaDispositivoId($data['marcaDispositivoId'])
                  ->setImei($data['imei'])
        ...
        } else if ($data instanceof \Atezate\Model\Raw\Dispositivos) {
            $entry->setId($data->getId())
                  ->setMarcaDispositivoId($data->getMarcaDispositivoId())
                  ->setImei($data->getImei())
                  ->setFechaCompra($data->getFechaCompra());
