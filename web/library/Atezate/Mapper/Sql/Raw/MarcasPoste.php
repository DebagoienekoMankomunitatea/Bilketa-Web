...
/**
 * Data Mapper implementation for Atezate\Model\MarcasPoste
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
class MarcasPoste extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\MarcasPoste';
     ...
     *
     * @param Atezate\Model\Raw\MarcasPoste $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\MarcasPoste) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\MarcasPoste object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\MarcasPoste object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\MarcasPoste
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\MarcasPoste');
        }
     ...
     *
     * @param Atezate\Model\Raw\MarcasPoste $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\MarcasPoste) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\MarcasPoste object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\MarcasPoste object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\MarcasPoste', $capitalizedFk);
     ...
     */
    public function save(\Atezate\Model\Raw\MarcasPoste $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\MarcasPoste $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Raw\MarcasPoste $model,
    ...
    )
    {
        ...
        try {
            ...
            if ($recursive) {
                if ($model->getPostesTipos(null, null, true) !== null) {
                    $postesTipos = $model->getPostesTipos();

                    if (!is_array($postesTipos)) {

                        $postesTipos = array($postesTipos);
                    }

                    foreach ($postesTipos as $value) {
                        $value->setMarcaPosteId($primaryKey)
     ...
     * @param Atezate\Model\Raw\MarcasPoste|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\MarcasPoste The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\MarcasPoste();
        }
        ...
        } else if ($data instanceof \Atezate\Model\Raw\MarcasPoste) {
