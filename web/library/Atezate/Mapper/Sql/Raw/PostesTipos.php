...
/**
 * Data Mapper implementation for Atezate\Model\PostesTipos
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
class PostesTipos extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\PostesTipos';
     ...
     *
     * @param Atezate\Model\Raw\PostesTipos $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\PostesTipos) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\PostesTipos object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\PostesTipos object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            'postesTiposId' => $model->getPostesTiposId(),
            'columna' => $model->getColumna(),
            'capacidad' => $model->getCapacidad(),
            'caras' => $model->getCaras(),
            'marcaPosteId' => $model->getMarcaPosteId(),
        );
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\PostesTipos
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\PostesTipos');
        }
     ...
     *
     * @param Atezate\Model\Raw\PostesTipos $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\PostesTipos) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\PostesTipos object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\PostesTipos object in delete for " . get_class($this);
            }
        ...
        try {
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\PostesTipos', $capitalizedFk);
            ...
            $where = $dbAdapter->quoteInto($dbAdapter->quoteIdentifier('postesTiposId') . ' = ?', $model->getPostesTiposId());
     ...
     */
    public function save(\Atezate\Model\Raw\PostesTipos $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\PostesTipos $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Raw\PostesTipos $model,
    ...
    )
    {
        ...
        $primaryKey = $model->getPostesTiposId();
        $success = true;
        ...
        try {
            if (is_null($primaryKey) || empty($primaryKey)) {
                ...
                if ($primaryKey) {
                    $model->setPostesTiposId($primaryKey);
                } else {
                ...
                $this->getDbTable()
                     ->update(
                         $data,
                         array(
                             $this->getDbTable()->getAdapter()->quoteIdentifier('postesTiposId') . ' = ?' => $primaryKey
                         )
                     );
            }
            ...
            if ($recursive) {
                if ($model->getPostes(null, null, true) !== null) {
                    ...
                    foreach ($postes as $value) {
                        $value->setPostesTiposId($primaryKey)
     ...
     * @param Atezate\Model\Raw\PostesTipos|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\PostesTipos The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\PostesTipos();
        }
        ...
        if (is_array($data)) {
            $entry->setPostesTiposId($data['postesTiposId'])
                  ->setColumna($data['columna'])
                  ->setCapacidad($data['capacidad'])
                  ->setCaras($data['caras'])
                  ->setMarcaPosteId($data['marcaPosteId']);
        ...
        } else if ($data instanceof \Atezate\Model\Raw\PostesTipos) {
            $entry->setPostesTiposId($data->getPostesTiposId())
                  ->setColumna($data->getColumna())
                  ->setCapacidad($data->getCapacidad())
                  ->setCaras($data->getCaras())
                  ->setMarcaPosteId($data->getMarcaPosteId());
