...
/**
 * Data Mapper implementation for Atezate\Model\CodigosApertura
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
class CodigosApertura extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\CodigosApertura';
     ...
     *
     * @param Atezate\Model\Raw\CodigosApertura $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\CodigosApertura) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\CodigosApertura object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\CodigosApertura object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            'id' => $model->getId(),
            'fechaLlamada' => $model->getFechaLlamada(),
            'contribuyenteId' => $model->getContribuyenteId(),
            'codigoApertura' => $model->getCodigoApertura(),
            'centroEmergenciaId' => $model->getCentroEmergenciaId(),
            'municipioId' => $model->getMunicipioId(),
            'activado' => $model->getActivado(),
        );
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\CodigosApertura
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\CodigosApertura');
        }
     ...
     *
     * @param Atezate\Model\Raw\CodigosApertura $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\CodigosApertura) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\CodigosApertura object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\CodigosApertura object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\CodigosApertura', $capitalizedFk);
     ...
     */
    public function save(\Atezate\Model\Raw\CodigosApertura $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\CodigosApertura $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Raw\CodigosApertura $model,
     ...
     * @param Atezate\Model\Raw\CodigosApertura|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\CodigosApertura The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\CodigosApertura();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ->setFechaLlamada($data['fechaLlamada'])
                  ->setContribuyenteId($data['contribuyenteId'])
                  ->setCodigoApertura($data['codigoApertura'])
                  ->setCentroEmergenciaId($data['centroEmergenciaId'])
                  ->setMunicipioId($data['municipioId'])
                  ->setActivado($data['activado']);
        ...
        } else if ($data instanceof \Atezate\Model\Raw\CodigosApertura) {
            $entry->setId($data->getId())
                  ->setFechaLlamada($data->getFechaLlamada())
                  ->setContribuyenteId($data->getContribuyenteId())
                  ->setCodigoApertura($data->getCodigoApertura())
                  ->setCentroEmergenciaId($data->getCentroEmergenciaId())
                  ->setMunicipioId($data->getMunicipioId())
                  ->setActivado($data->getActivado());
