...
/**
 * Data Mapper implementation for Atezate\Model\Telefonos
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
class Telefonos extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\Telefonos';
     ...
     *
     * @param Atezate\Model\Raw\Telefonos $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\Telefonos) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\Telefonos object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\Telefonos object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            'telefonoId' => $model->getTelefonoId(),
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\Telefonos
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\Telefonos');
        }
     ...
     *
     * @param Atezate\Model\Raw\Telefonos $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\Telefonos) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\Telefonos object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\Telefonos object in delete for " . get_class($this);
            }
        ...
        try {
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\Telefonos', $capitalizedFk);
            ...
            $where = $dbAdapter->quoteInto($dbAdapter->quoteIdentifier('telefonoId') . ' = ?', $model->getTelefonoId());
     ...
     */
    public function save(\Atezate\Model\Raw\Telefonos $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\Telefonos $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Raw\Telefonos $model,
    ...
    )
    {
        ...
        $primaryKey = $model->getTelefonoId();
        $success = true;
        ...
        try {
            if (is_null($primaryKey) || empty($primaryKey)) {
                ...
                if ($primaryKey) {
                    $model->setTelefonoId($primaryKey);
                } else {
                ...
                $this->getDbTable()
                     ->update(
                         $data,
                         array(
                             $this->getDbTable()->getAdapter()->quoteIdentifier('telefonoId') . ' = ?' => $primaryKey
                         )
                     );
            }
     ...
     * @param Atezate\Model\Raw\Telefonos|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\Telefonos The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\Telefonos();
        }
        ...
        if (is_array($data)) {
            $entry->setTelefonoId($data['telefonoId'])
        ...
        } else if ($data instanceof \Atezate\Model\Raw\Telefonos) {
            $entry->setTelefonoId($data->getTelefonoId())
