...
/**
 * Data Mapper implementation for Atezate\Model\CodigoApertura
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
class CodigoApertura extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\CodigoApertura';
     ...
     *
     * @param Atezate\Model\Raw\CodigoApertura $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\CodigoApertura) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\CodigoApertura object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\CodigoApertura object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\CodigoApertura
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\CodigoApertura');
        }
     ...
     *
     * @param Atezate\Model\Raw\CodigoApertura $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\CodigoApertura) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\CodigoApertura object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\CodigoApertura object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\CodigoApertura', $capitalizedFk);
     ...
     */
    public function save(\Atezate\Model\Raw\CodigoApertura $model)
    {
     ...
     * @return boolean If the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\CodigoApertura $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Raw\CodigoApertura $model,
     ...
     * @param Atezate\Model\Raw\CodigoApertura|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\CodigoApertura The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\CodigoApertura();
        }
