...
/**
 * Data Mapper implementation for Atezate\Model\MunicipiosRelCodigosPostales
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
class MunicipiosRelCodigosPostales extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\MunicipiosRelCodigosPostales';
     ...
     *
     * @param Atezate\Model\Raw\MunicipiosRelCodigosPostales $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\MunicipiosRelCodigosPostales) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\MunicipiosRelCodigosPostales object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\MunicipiosRelCodigosPostales object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            ...
            'codigoPostalId' => $model->getCodigoPostalId(),
        );
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\MunicipiosRelCodigosPostales
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\MunicipiosRelCodigosPostales');
        }
     ...
     *
     * @param Atezate\Model\Raw\MunicipiosRelCodigosPostales $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\MunicipiosRelCodigosPostales) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\MunicipiosRelCodigosPostales object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\MunicipiosRelCodigosPostales object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\MunicipiosRelCodigosPostales', $capitalizedFk);
     ...
     */
    public function save(\Atezate\Model\Raw\MunicipiosRelCodigosPostales $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\MunicipiosRelCodigosPostales $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Raw\MunicipiosRelCodigosPostales $model,
     ...
     * @param Atezate\Model\Raw\MunicipiosRelCodigosPostales|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\MunicipiosRelCodigosPostales The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\MunicipiosRelCodigosPostales();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ...
                  ->setCodigoPostalId($data['codigoPostalId']);
        } else if ($data instanceof \Zend_Db_Table_Row_Abstract || $data instanceof \stdClass) {
            $entry->setId($data->{'id'})
                  ->setMunicipioId($data->{'municipioId'})
                  ->setCodigoPostalId($data->{'codigoPostalId'});

        } else if ($data instanceof \Atezate\Model\Raw\MunicipiosRelCodigosPostales) {
            $entry->setId($data->getId())
                  ->setMunicipioId($data->getMunicipioId())
                  ->setCodigoPostalId($data->getCodigoPostalId());
