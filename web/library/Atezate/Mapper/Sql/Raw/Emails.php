...
/**
 * Data Mapper implementation for Atezate\Model\Emails
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
class Emails extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\Emails';
     ...
     *
     * @param Atezate\Model\Raw\Emails $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\Emails) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\Emails object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\Emails object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\Emails
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\Emails');
        }
     ...
     *
     * @param Atezate\Model\Raw\Emails $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\Emails) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\Emails object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\Emails object in delete for " . get_class($this);
            }
        ...
        try {
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\Emails', $capitalizedFk);
            ...
            $where = $dbAdapter->quoteInto($dbAdapter->quoteIdentifier('emailId') . ' = ?', $model->getEmailId());
     ...
     */
    public function save(\Atezate\Model\Raw\Emails $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\Emails $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Raw\Emails $model,
            ...
            if (is_null($primaryKey) || empty($primaryKey)) {
                ...
                $this->getDbTable()
                     ->update(
                         $data,
                         array(
                             $this->getDbTable()->getAdapter()->quoteIdentifier('emailId') . ' = ?' => $primaryKey
                         )
                     );
            }
     ...
     * @param Atezate\Model\Raw\Emails|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\Emails The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\Emails();
        }
        ...
        } else if ($data instanceof \Atezate\Model\Raw\Emails) {
            $entry->setEmailId($data->getEmailId())
