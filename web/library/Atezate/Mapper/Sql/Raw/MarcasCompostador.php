...
/**
 * Data Mapper implementation for Atezate\Model\MarcasCompostador
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
class MarcasCompostador extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\MarcasCompostador';
     ...
     *
     * @param Atezate\Model\Raw\MarcasCompostador $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\MarcasCompostador) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\MarcasCompostador object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\MarcasCompostador object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\MarcasCompostador
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\MarcasCompostador');
        }
     ...
     *
     * @param Atezate\Model\Raw\MarcasCompostador $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\MarcasCompostador) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\MarcasCompostador object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\MarcasCompostador object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\MarcasCompostador', $capitalizedFk);
     ...
     */
    public function save(\Atezate\Model\Raw\MarcasCompostador $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\MarcasCompostador $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Raw\MarcasCompostador $model,
    ...
    )
    {
        ...
        try {
            ...
            if ($recursive) {
                if ($model->getCompostadores(null, null, true) !== null) {
                    $compostadores = $model->getCompostadores();

                    if (!is_array($compostadores)) {

                        $compostadores = array($compostadores);
                    }

                    foreach ($compostadores as $value) {
                        $value->setMarcaCompostadorId($primaryKey)
     ...
     * @param Atezate\Model\Raw\MarcasCompostador|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\MarcasCompostador The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\MarcasCompostador();
        }
        ...
        } else if ($data instanceof \Atezate\Model\Raw\MarcasCompostador) {
            $entry->setId($data->getId())
                  ->setMarca($data->getMarca())
