...
/**
 * Data Mapper implementation for Atezate\Model\Rutas
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
class Rutas extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\Rutas';
     ...
     *
     * @param Atezate\Model\Raw\Rutas $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\Rutas) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\Rutas object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\Rutas object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            ...
            'rutasTiposId' => $model->getRutasTiposId(),
            'tiempoAprox' => $model->getTiempoAprox(),
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\Rutas
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\Rutas');
        }
     ...
     *
     * @param Atezate\Model\Raw\Rutas $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\Rutas) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\Rutas object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\Rutas object in delete for " . get_class($this);
            }
        ...
        try {
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\Rutas', $capitalizedFk);
            ...
            $where = $dbAdapter->quoteInto($dbAdapter->quoteIdentifier('rutaId') . ' = ?', $model->getRutaId());
     ...
     */
    public function save(\Atezate\Model\Raw\Rutas $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\Rutas $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Raw\Rutas $model,
            ...
            if (is_null($primaryKey) || empty($primaryKey)) {
                ...
                $this->getDbTable()
                     ->update(
                         $data,
                         array(
                             $this->getDbTable()->getAdapter()->quoteIdentifier('rutaId') . ' = ?' => $primaryKey
                         )
                     );
            }
            ...
            if ($recursive) {
                if ($model->getRutasRelPuntosRecogida(null, null, true) !== null) {
                    ...
                    foreach ($rutasRelPuntosRecogida as $value) {
                        $value->setRutaId($primaryKey)
                              ->saveRecursive(false, $transactionTag);
                    }
                }

                if ($model->getTurnos(null, null, true) !== null) {
                    $turnos = $model->getTurnos();
     ...
     * @param Atezate\Model\Raw\Rutas|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\Rutas The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\Rutas();
        }
        ...
        if (is_array($data)) {
            $entry->setRutaId($data['rutaId'])
                  ->setIdentificacion($data['identificacion'])
                  ->setRutasTiposId($data['rutasTiposId'])
                  ->setTiempoAprox($data['tiempoAprox'])
                  ->setPuntosDescargaId($data['puntosDescargaId'])
        ...
        } else if ($data instanceof \Atezate\Model\Raw\Rutas) {
            $entry->setRutaId($data->getRutaId())
                  ->setIdentificacion($data->getIdentificacion())
                  ->setRutasTiposId($data->getRutasTiposId())
                  ->setTiempoAprox($data->getTiempoAprox())
                  ->setPuntosDescargaId($data->getPuntosDescargaId())
