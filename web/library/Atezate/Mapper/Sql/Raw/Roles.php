...
/**
 * Data Mapper implementation for Atezate\Model\Roles
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
class Roles extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\Roles';
     ...
     *
     * @param Atezate\Model\Raw\Roles $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\Roles) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\Roles object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\Roles object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\Roles
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\Roles');
        }
     ...
     *
     * @param Atezate\Model\Raw\Roles $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\Roles) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\Roles object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\Roles object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\Roles', $capitalizedFk);
     ...
     */
    public function save(\Atezate\Model\Raw\Roles $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\Roles $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Raw\Roles $model,
    ...
    )
    {
        ...
        try {
            ...
            if ($recursive) {
                if ($model->getOperarios(null, null, true) !== null) {
                    $operarios = $model->getOperarios();

                    if (!is_array($operarios)) {

                        $operarios = array($operarios);
                    }

                    foreach ($operarios as $value) {
                        $value->setRolId($primaryKey)
     ...
     * @param Atezate\Model\Raw\Roles|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\Roles The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\Roles();
        }
        ...
        } else if ($data instanceof \Atezate\Model\Raw\Roles) {
