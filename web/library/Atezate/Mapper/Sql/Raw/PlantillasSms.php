...
/**
 * Data Mapper implementation for Atezate\Model\PlantillasSms
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
class PlantillasSms extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\PlantillasSms';
     ...
     *
     * @param Atezate\Model\Raw\PlantillasSms $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\PlantillasSms) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\PlantillasSms object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\PlantillasSms object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\PlantillasSms
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\PlantillasSms');
        }
     ...
     *
     * @param Atezate\Model\Raw\PlantillasSms $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\PlantillasSms) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\PlantillasSms object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\PlantillasSms object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\PlantillasSms', $capitalizedFk);
     ...
     */
    public function save(\Atezate\Model\Raw\PlantillasSms $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\PlantillasSms $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Raw\PlantillasSms $model,
            ...
            if ($recursive) {
                if ($model->getLogSms(null, null, true) !== null) {
                    ...
                    foreach ($logSms as $value) {
                        $value->setPlantillasSmsId($primaryKey)
     ...
     * @param Atezate\Model\Raw\PlantillasSms|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\PlantillasSms The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\PlantillasSms();
        }
        ...
        } else if ($data instanceof \Atezate\Model\Raw\PlantillasSms) {
