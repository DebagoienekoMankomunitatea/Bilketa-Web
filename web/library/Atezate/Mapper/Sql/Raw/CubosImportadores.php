...
class CubosImportadores extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\CubosImportadores';
     ...
     *
     * @param Atezate\Model\Raw\CubosImportadores $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\CubosImportadores) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\CubosImportadores object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\CubosImportadores object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            'id' => $model->getId(),
            'csvFileSize' => $model->getCsvFileSize(),
            'csvMimeType' => $model->getCsvMimeType(),
            'csvBaseName' => $model->getCsvBaseName(),
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\CubosImportadores
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\CubosImportadores');
        }
     ...
     *
     * @param Atezate\Model\Raw\CubosImportadores $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\CubosImportadores) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\CubosImportadores object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\CubosImportadores object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\CubosImportadores', $capitalizedFk);
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\CubosImportadores $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Raw\CubosImportadores $model,
     ...
     * @param Atezate\Model\Raw\CubosImportadores|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\CubosImportadores The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\CubosImportadores();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ->setCsvFileSize($data['csvFileSize'])
                  ->setCsvMimeType($data['csvMimeType'])
                  ->setCsvBaseName($data['csvBaseName'])
        ...
        } else if ($data instanceof \Atezate\Model\Raw\CubosImportadores) {
            $entry->setId($data->getId())
                  ->setCsvFileSize($data->getCsvFileSize())
                  ->setCsvMimeType($data->getCsvMimeType())
                  ->setCsvBaseName($data->getCsvBaseName())
