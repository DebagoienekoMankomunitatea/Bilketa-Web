...
/**
 * Data Mapper implementation for Atezate\Model\MarcasDispositivo
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
class MarcasDispositivo extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\MarcasDispositivo';
     ...
     *
     * @param Atezate\Model\Raw\MarcasDispositivo $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\MarcasDispositivo) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\MarcasDispositivo object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\MarcasDispositivo object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\MarcasDispositivo
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\MarcasDispositivo');
        }
     ...
     *
     * @param Atezate\Model\Raw\MarcasDispositivo $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\MarcasDispositivo) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\MarcasDispositivo object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\MarcasDispositivo object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\MarcasDispositivo', $capitalizedFk);
     ...
     */
    public function save(\Atezate\Model\Raw\MarcasDispositivo $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\MarcasDispositivo $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Raw\MarcasDispositivo $model,
    ...
    )
    {
        ...
        try {
            ...
            if ($recursive) {
                if ($model->getDispositivos(null, null, true) !== null) {
                    $dispositivos = $model->getDispositivos();

                    if (!is_array($dispositivos)) {

                        $dispositivos = array($dispositivos);
                    }

                    foreach ($dispositivos as $value) {
                        $value->setMarcaDispositivoId($primaryKey)
     ...
     * @param Atezate\Model\Raw\MarcasDispositivo|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\MarcasDispositivo The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\MarcasDispositivo();
        }
        ...
        } else if ($data instanceof \Atezate\Model\Raw\MarcasDispositivo) {
