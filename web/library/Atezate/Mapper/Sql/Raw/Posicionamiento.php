...
class Posicionamiento extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\Posicionamiento';
     ...
     *
     * @param Atezate\Model\Raw\Posicionamiento $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\Posicionamiento) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\Posicionamiento object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\Posicionamiento object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            'posicionamientoId' => $model->getPosicionamientoId(),
            'turnosCamionesId' => $model->getTurnosCamionesId(),
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\Posicionamiento
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\Posicionamiento');
        }
     ...
     *
     * @param Atezate\Model\Raw\Posicionamiento $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\Posicionamiento) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\Posicionamiento object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\Posicionamiento object in delete for " . get_class($this);
            }
        ...
        try {
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\Posicionamiento', $capitalizedFk);
            ...
            $where = $dbAdapter->quoteInto($dbAdapter->quoteIdentifier('posicionamientoId') . ' = ?', $model->getPosicionamientoId());
     ...
     */
    public function save(\Atezate\Model\Raw\Posicionamiento $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\Posicionamiento $model, $useTransaction = true, $transactionTag = null)
    {
    ...
    )
    {
        ...
        $primaryKey = $model->getPosicionamientoId();
        $success = true;
        ...
        try {
            if (is_null($primaryKey) || empty($primaryKey)) {
                ...
                if ($primaryKey) {
                    $model->setPosicionamientoId($primaryKey);
                } else {
                ...
                $this->getDbTable()
                     ->update(
                         $data,
                         array(
                             $this->getDbTable()->getAdapter()->quoteIdentifier('posicionamientoId') . ' = ?' => $primaryKey
                         )
                     );
            }
     ...
     * @param Atezate\Model\Raw\Posicionamiento|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\Posicionamiento The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\Posicionamiento();
        }
        ...
        if (is_array($data)) {
            $entry->setPosicionamientoId($data['posicionamientoId'])
                  ->setTurnosCamionesId($data['turnosCamionesId'])
        ...
        } else if ($data instanceof \Atezate\Model\Raw\Posicionamiento) {
            $entry->setPosicionamientoId($data->getPosicionamientoId())
                  ->setTurnosCamionesId($data->getTurnosCamionesId())
                  ->setPrecision($data->getPrecision())
                  ->setFecha($data->getFecha())
