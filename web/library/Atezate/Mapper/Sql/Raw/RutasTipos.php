...
/**
 * Data Mapper implementation for Atezate\Model\RutasTipos
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
class RutasTipos extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\RutasTipos';
     ...
     *
     * @param Atezate\Model\Raw\RutasTipos $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\RutasTipos) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\RutasTipos object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\RutasTipos object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\RutasTipos
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\RutasTipos');
        }
     ...
     *
     * @param Atezate\Model\Raw\RutasTipos $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\RutasTipos) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\RutasTipos object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\RutasTipos object in delete for " . get_class($this);
            }
        ...
        try {
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\RutasTipos', $capitalizedFk);
            ...
            $where = $dbAdapter->quoteInto($dbAdapter->quoteIdentifier('rutasTiposId') . ' = ?', $model->getRutasTiposId());
     ...
     */
    public function save(\Atezate\Model\Raw\RutasTipos $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\RutasTipos $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Raw\RutasTipos $model,
    ...
    )
    {
        ...
        $primaryKey = $model->getRutasTiposId();
        $success = true;
        ...
        try {
            if (is_null($primaryKey) || empty($primaryKey)) {
                ...
                if ($primaryKey) {
                    $model->setRutasTiposId($primaryKey);
                } else {
                ...
                $this->getDbTable()
                     ->update(
                         $data,
                         array(
                             $this->getDbTable()->getAdapter()->quoteIdentifier('rutasTiposId') . ' = ?' => $primaryKey
                         )
                     );
            }
            ...
            if ($recursive) {
                if ($model->getRutas(null, null, true) !== null) {
                    ...
                    foreach ($rutas as $value) {
                        $value->setRutasTiposId($primaryKey)
     ...
     * @param Atezate\Model\Raw\RutasTipos|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\RutasTipos The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\RutasTipos();
        }
        ...
        if (is_array($data)) {
            $entry->setRutasTiposId($data['rutasTiposId'])
        ...
        } else if ($data instanceof \Atezate\Model\Raw\RutasTipos) {
            $entry->setRutasTiposId($data->getRutasTiposId())
