...
/**
 * Data Mapper implementation for Atezate\Model\Descargas
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
class Descargas extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\Descargas';
     ...
     *
     * @param Atezate\Model\Raw\Descargas $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\Descargas) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\Descargas object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\Descargas object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            ...
            'puntoDescargaId' => $model->getPuntoDescargaId(),
            'kilos' => $model->getKilos(),
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\Descargas
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\Descargas');
        }
     ...
     *
     * @param Atezate\Model\Raw\Descargas $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\Descargas) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\Descargas object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\Descargas object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\Descargas', $capitalizedFk);
     ...
     */
    public function save(\Atezate\Model\Raw\Descargas $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\Descargas $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Raw\Descargas $model,
     ...
     * @param Atezate\Model\Raw\Descargas|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\Descargas The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\Descargas();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ...
                  ->setPuntoDescargaId($data['puntoDescargaId'])
                  ->setKilos($data['kilos'])
        ...
        } else if ($data instanceof \Atezate\Model\Raw\Descargas) {
            $entry->setId($data->getId())
                  ->setCamionId($data->getCamionId())
                  ->setPuntoDescargaId($data->getPuntoDescargaId())
                  ->setKilos($data->getKilos())
