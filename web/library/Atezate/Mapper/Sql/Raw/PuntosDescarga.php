...
class PuntosDescarga extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\PuntosDescarga';
     ...
     *
     * @param Atezate\Model\Raw\PuntosDescarga $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\PuntosDescarga) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\PuntosDescarga object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\PuntosDescarga object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            'puntosDescargaId' => $model->getPuntosDescargaId(),
            'puntosDescargaIden' => $model->getPuntosDescargaIden(),
            'residuosTiposId' => $model->getResiduosTiposId(),
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\PuntosDescarga
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\PuntosDescarga');
        }
     ...
     *
     * @param Atezate\Model\Raw\PuntosDescarga $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\PuntosDescarga) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\PuntosDescarga object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\PuntosDescarga object in delete for " . get_class($this);
            }
        ...
        try {
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\PuntosDescarga', $capitalizedFk);
            ...
            $where = $dbAdapter->quoteInto($dbAdapter->quoteIdentifier('puntosDescargaId') . ' = ?', $model->getPuntosDescargaId());
     ...
     */
    public function save(\Atezate\Model\Raw\PuntosDescarga $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\PuntosDescarga $model, $useTransaction = true, $transactionTag = null)
    {
    ...
    )
    {
        ...
        $primaryKey = $model->getPuntosDescargaId();
        $success = true;
        ...
        try {
            if (is_null($primaryKey) || empty($primaryKey)) {
                ...
                if ($primaryKey) {
                    $model->setPuntosDescargaId($primaryKey);
                } else {
                ...
                $this->getDbTable()
                     ->update(
                         $data,
                         array(
                             $this->getDbTable()->getAdapter()->quoteIdentifier('puntosDescargaId') . ' = ?' => $primaryKey
                         )
                     );
            }
            ...
            if ($recursive) {
                if ($model->getDescargas(null, null, true) !== null) {
                    $descargas = $model->getDescargas();

                    if (!is_array($descargas)) {

                        $descargas = array($descargas);
                    }

                    foreach ($descargas as $value) {
                        $value->setPuntoDescargaId($primaryKey)
                              ->saveRecursive(false, $transactionTag);
                    }
                }

                if ($model->getRutas(null, null, true) !== null) {
                    ...
                    foreach ($rutas as $value) {
                        $value->setPuntosDescargaId($primaryKey)
                              ->saveRecursive(false, $transactionTag);
                    }
                }
     ...
     * @param Atezate\Model\Raw\PuntosDescarga|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\PuntosDescarga The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\PuntosDescarga();
        }
        ...
        if (is_array($data)) {
            $entry->setPuntosDescargaId($data['puntosDescargaId'])
                  ->setPuntosDescargaIden($data['puntosDescargaIden'])
                  ->setResiduosTiposId($data['residuosTiposId'])
        ...
        } else if ($data instanceof \Atezate\Model\Raw\PuntosDescarga) {
            $entry->setPuntosDescargaId($data->getPuntosDescargaId())
                  ->setPuntosDescargaIden($data->getPuntosDescargaIden())
                  ->setResiduosTiposId($data->getResiduosTiposId())
