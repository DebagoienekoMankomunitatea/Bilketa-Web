...
/**
 * Data Mapper implementation for Atezate\Model\RevisionTipos
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
class RevisionTipos extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\RevisionTipos';
     ...
     *
     * @param Atezate\Model\Raw\RevisionTipos $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\RevisionTipos) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\RevisionTipos object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\RevisionTipos object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\RevisionTipos
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\RevisionTipos');
        }
     ...
     *
     * @param Atezate\Model\Raw\RevisionTipos $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\RevisionTipos) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\RevisionTipos object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\RevisionTipos object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\RevisionTipos', $capitalizedFk);
     ...
     */
    public function save(\Atezate\Model\Raw\RevisionTipos $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\RevisionTipos $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Raw\RevisionTipos $model,
            ...
            if ($recursive) {
                if ($model->getRevision(null, null, true) !== null) {
                    ...
                    foreach ($revision as $value) {
                        $value->setRevisionTipoId($primaryKey)
     ...
     * @param Atezate\Model\Raw\RevisionTipos|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\RevisionTipos The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\RevisionTipos();
        }
        ...
        } else if ($data instanceof \Atezate\Model\Raw\RevisionTipos) {
