...
/**
 * Data Mapper implementation for Atezate\Model\Puertas
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
class Puertas extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\Puertas';
     ...
     *
     * @param Atezate\Model\Puertas $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Puertas) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Puertas object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\Puertas object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\Puertas
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\Puertas');
        }
     ...
     *
     * @param Atezate\Model\Puertas $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Puertas) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\Puertas object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\Puertas object in delete for " . get_class($this);
            }
        ...
        try {
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\Puertas', $capitalizedFk);
            ...
            $where = $dbAdapter->quoteInto($dbAdapter->quoteIdentifier('puertasId') . ' = ?', $model->getPuertasId());
     ...
     */
    public function save(\Atezate\Model\Puertas $model)
    {
     ...
     * @return boolean If the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Puertas $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Puertas $model,
    ...
    )
    {
        ...
        unset($data['puertasId']);

        try {
            if (is_null($primaryKey) || empty($primaryKey)) {
                $primaryKey = $this->getDbTable()->insert($data);
                if ($primaryKey) {
                    $model->setPuertasId($primaryKey);
                } else {
                ...
                $this->getDbTable()
                     ->update(
                         $data,
                         array(
                             $this->getDbTable()->getAdapter()->quoteIdentifier('puertasId') . ' = ?' => $primaryKey
                         )
                     );
            }
     ...
     * @param Atezate\Model\Puertas|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Puertas The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\Puertas();
        }
        ...
        if (is_array($data)) {
            $entry->setPuertasId($data['puertasId'])
        ...
        } else if ($data instanceof \Atezate\Model\Puertas) {
            $entry->setPuertasId($data->getPuertasId())
