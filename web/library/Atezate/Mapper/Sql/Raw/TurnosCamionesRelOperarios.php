...
/**
 * Data Mapper implementation for Atezate\Model\TurnosCamionesRelOperarios
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
class TurnosCamionesRelOperarios extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\TurnosCamionesRelOperarios';
     ...
     *
     * @param Atezate\Model\Raw\TurnosCamionesRelOperarios $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\TurnosCamionesRelOperarios) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\TurnosCamionesRelOperarios object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\TurnosCamionesRelOperarios object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            'id' => $model->getId(),
            'turnosRelCamionesId' => $model->getTurnosRelCamionesId(),
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\TurnosCamionesRelOperarios
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\TurnosCamionesRelOperarios');
        }
     ...
     *
     * @param Atezate\Model\Raw\TurnosCamionesRelOperarios $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\TurnosCamionesRelOperarios) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\TurnosCamionesRelOperarios object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\TurnosCamionesRelOperarios object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\TurnosCamionesRelOperarios', $capitalizedFk);
     ...
     */
    public function save(\Atezate\Model\Raw\TurnosCamionesRelOperarios $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\TurnosCamionesRelOperarios $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Raw\TurnosCamionesRelOperarios $model,
     ...
     * @param Atezate\Model\Raw\TurnosCamionesRelOperarios|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\TurnosCamionesRelOperarios The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\TurnosCamionesRelOperarios();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ->setTurnosRelCamionesId($data['turnosRelCamionesId'])
        ...
        } else if ($data instanceof \Atezate\Model\Raw\TurnosCamionesRelOperarios) {
            $entry->setId($data->getId())
                  ->setTurnosRelCamionesId($data->getTurnosRelCamionesId())
                  ->setOperarioId($data->getOperarioId());
