...
/**
 * Data Mapper implementation for Atezate\Model\LogSms
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
class LogSms extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\LogSms';
     ...
     *
     * @param Atezate\Model\Raw\LogSms $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\LogSms) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\LogSms object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\LogSms object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            'smsMensajeId' => $model->getSmsMensajeId(),
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\LogSms
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\LogSms');
        }
     ...
     *
     * @param Atezate\Model\Raw\LogSms $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\LogSms) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\LogSms object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\LogSms object in delete for " . get_class($this);
            }
        ...
        try {
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\LogSms', $capitalizedFk);
            ...
            $where = $dbAdapter->quoteInto($dbAdapter->quoteIdentifier('smsMensajeId') . ' = ?', $model->getSmsMensajeId());
     ...
     */
    public function save(\Atezate\Model\Raw\LogSms $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\LogSms $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Raw\LogSms $model,
    ...
    )
    {
        ...
        $primaryKey = $model->getSmsMensajeId();
        $success = true;
        ...
        try {
            if (is_null($primaryKey) || empty($primaryKey)) {
                ...
                if ($primaryKey) {
                    $model->setSmsMensajeId($primaryKey);
                } else {
                ...
                $this->getDbTable()
                     ->update(
                         $data,
                         array(
                             $this->getDbTable()->getAdapter()->quoteIdentifier('smsMensajeId') . ' = ?' => $primaryKey
                         )
                     );
            }
     ...
     * @param Atezate\Model\Raw\LogSms|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\LogSms The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\LogSms();
        }
        ...
        if (is_array($data)) {
            $entry->setSmsMensajeId($data['smsMensajeId'])
        ...
        } else if ($data instanceof \Atezate\Model\Raw\LogSms) {
            $entry->setSmsMensajeId($data->getSmsMensajeId())
                  ->setMensaje($data->getMensaje())
