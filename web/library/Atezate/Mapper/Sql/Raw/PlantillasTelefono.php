...
/**
 * Data Mapper implementation for Atezate\Model\PlantillasTelefono
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
class PlantillasTelefono extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\PlantillasTelefono';
     ...
     *
     * @param Atezate\Model\Raw\PlantillasTelefono $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\PlantillasTelefono) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\PlantillasTelefono object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\PlantillasTelefono object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\PlantillasTelefono
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\PlantillasTelefono');
        }
     ...
     *
     * @param Atezate\Model\Raw\PlantillasTelefono $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\PlantillasTelefono) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\PlantillasTelefono object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\PlantillasTelefono object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\PlantillasTelefono', $capitalizedFk);
     ...
     */
    public function save(\Atezate\Model\Raw\PlantillasTelefono $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\PlantillasTelefono $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Raw\PlantillasTelefono $model,
            ...
            if ($recursive) {
                if ($model->getLogLlamadas(null, null, true) !== null) {
                    ...
                    foreach ($logLlamadas as $value) {
                        $value->setPlantillasTelefonoId($primaryKey)
     ...
     * @param Atezate\Model\Raw\PlantillasTelefono|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\PlantillasTelefono The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\PlantillasTelefono();
        }
        ...
        } else if ($data instanceof \Atezate\Model\Raw\PlantillasTelefono) {
