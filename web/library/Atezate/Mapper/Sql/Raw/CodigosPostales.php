...
/**
 * Data Mapper implementation for Atezate\Model\CodigosPostales
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
class CodigosPostales extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\CodigosPostales';
     ...
     *
     * @param Atezate\Model\Raw\CodigosPostales $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\CodigosPostales) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\CodigosPostales object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\CodigosPostales object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            'id' => $model->getId(),
            'codigoPostal' => $model->getCodigoPostal(),
        );
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\CodigosPostales
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\CodigosPostales');
        }
     ...
     *
     * @param Atezate\Model\Raw\CodigosPostales $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\CodigosPostales) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\CodigosPostales object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\CodigosPostales object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\CodigosPostales', $capitalizedFk);
     ...
     */
    public function save(\Atezate\Model\Raw\CodigosPostales $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\CodigosPostales $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Raw\CodigosPostales $model,
            ...
            if ($recursive) {
                if ($model->getMunicipiosRelCodigosPostales(null, null, true) !== null) {
                    ...
                    foreach ($municipiosRelCodigosPostales as $value) {
                        $value->setCodigoPostalId($primaryKey)
     ...
     * @param Atezate\Model\Raw\CodigosPostales|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\CodigosPostales The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\CodigosPostales();
        }
        ...
        } else if ($data instanceof \Atezate\Model\Raw\CodigosPostales) {
            $entry->setId($data->getId())
                  ->setCodigoPostal($data->getCodigoPostal());
