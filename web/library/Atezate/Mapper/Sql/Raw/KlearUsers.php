...
/**
 * Data Mapper implementation for Atezate\Model\KlearUsers
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
{
    protected $_modelName = 'Atezate\\Model\\KlearUsers';
     ...
     *
     * @param Atezate\Model\Raw\KlearUsers $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\KlearUsers) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\KlearUsers object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\KlearUsers object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\KlearUsers
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\KlearUsers');
        }
     ...
     *
     * @param Atezate\Model\Raw\KlearUsers $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\KlearUsers) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\KlearUsers object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\KlearUsers object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\KlearUsers', $capitalizedFk);
     ...
     */
    public function save(\Atezate\Model\Raw\KlearUsers $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\KlearUsers $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Raw\KlearUsers $model,
     ...
     * @param Atezate\Model\Raw\KlearUsers|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\KlearUsers The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\KlearUsers();
        }
        ...
        } else if ($data instanceof \Atezate\Model\Raw\KlearUsers) {
