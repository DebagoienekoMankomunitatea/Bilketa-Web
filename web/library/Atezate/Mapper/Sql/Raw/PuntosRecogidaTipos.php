...
/**
 * Data Mapper implementation for Atezate\Model\PuntosRecogidaTipos
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
class PuntosRecogidaTipos extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\PuntosRecogidaTipos';
     ...
     *
     * @param Atezate\Model\Raw\PuntosRecogidaTipos $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\PuntosRecogidaTipos) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\PuntosRecogidaTipos object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\PuntosRecogidaTipos object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\PuntosRecogidaTipos
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\PuntosRecogidaTipos');
        }
     ...
     *
     * @param Atezate\Model\Raw\PuntosRecogidaTipos $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\PuntosRecogidaTipos) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\PuntosRecogidaTipos object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\PuntosRecogidaTipos object in delete for " . get_class($this);
            }
        ...
        try {
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\PuntosRecogidaTipos', $capitalizedFk);
            ...
            $where = $dbAdapter->quoteInto($dbAdapter->quoteIdentifier('puntosRecogidaTiposId') . ' = ?', $model->getPuntosRecogidaTiposId());
     ...
     */
    public function save(\Atezate\Model\Raw\PuntosRecogidaTipos $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\PuntosRecogidaTipos $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Raw\PuntosRecogidaTipos $model,
    ...
    )
    {
        ...
        $primaryKey = $model->getPuntosRecogidaTiposId();
        $success = true;
        ...
        unset($data['puntosRecogidaTiposId']);
        ...
        try {
            if (is_null($primaryKey) || empty($primaryKey)) {
                ...
                if ($primaryKey) {
                    $model->setPuntosRecogidaTiposId($primaryKey);
                } else {
                ...
                $this->getDbTable()
                     ->update(
                         $data,
                         array(
                             $this->getDbTable()->getAdapter()->quoteIdentifier('puntosRecogidaTiposId') . ' = ?' => $primaryKey
                         )
                     );
            }
            ...
            if ($recursive) {
                if ($model->getPuntosRecogida(null, null, true) !== null) {
                    ...
                    foreach ($puntosRecogida as $value) {
                        $value->setPuntosRecogidaTiposId($primaryKey)
     ...
     * @param Atezate\Model\Raw\PuntosRecogidaTipos|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\PuntosRecogidaTipos The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\PuntosRecogidaTipos();
        }
        ...
        if (is_array($data)) {
            $entry->setPuntosRecogidaTiposId($data['puntosRecogidaTiposId'])
        ...
        } else if ($data instanceof \Atezate\Model\Raw\PuntosRecogidaTipos) {
            $entry->setPuntosRecogidaTiposId($data->getPuntosRecogidaTiposId())
