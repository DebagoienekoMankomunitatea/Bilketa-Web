...
/**
 * Data Mapper implementation for Atezate\Model\CompostadoresRelContribuyentes
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
class CompostadoresRelContribuyentes extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\CompostadoresRelContribuyentes';
     ...
     *
     * @param Atezate\Model\Raw\CompostadoresRelContribuyentes $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\CompostadoresRelContribuyentes) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\CompostadoresRelContribuyentes object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\CompostadoresRelContribuyentes object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\CompostadoresRelContribuyentes
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\CompostadoresRelContribuyentes');
        }
     ...
     *
     * @param Atezate\Model\Raw\CompostadoresRelContribuyentes $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\CompostadoresRelContribuyentes) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\CompostadoresRelContribuyentes object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\CompostadoresRelContribuyentes object in delete for " . get_class($this);
            }
        ...
        try {
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\CompostadoresRelContribuyentes', $capitalizedFk);
            ...
            $where = $dbAdapter->quoteInto($dbAdapter->quoteIdentifier('idRel') . ' = ?', $model->getIdRel());
     ...
     */
    public function save(\Atezate\Model\Raw\CompostadoresRelContribuyentes $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\CompostadoresRelContribuyentes $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Raw\CompostadoresRelContribuyentes $model,
            ...
            if (is_null($primaryKey) || empty($primaryKey)) {
                ...
                $this->getDbTable()
                     ->update(
                         $data,
                         array(
                             $this->getDbTable()->getAdapter()->quoteIdentifier('idRel') . ' = ?' => $primaryKey
                         )
                     );
            }
     ...
     * @param Atezate\Model\Raw\CompostadoresRelContribuyentes|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\CompostadoresRelContribuyentes The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\CompostadoresRelContribuyentes();
        }
        ...
        if (is_array($data)) {
            $entry->setIdRel($data['idRel'])
                  ->setCompostadoresId($data['compostadoresId'])
        ...
        } else if ($data instanceof \Atezate\Model\Raw\CompostadoresRelContribuyentes) {
            $entry->setIdRel($data->getIdRel())
                  ->setCompostadoresId($data->getCompostadoresId())
