...
class Camiones extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\Camiones';
     ...
     *
     * @param Atezate\Model\Raw\Camiones $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\Camiones) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\Camiones object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\Camiones object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            ...
            'cuandoPosicion' => $model->getCuandoPosicion(),
            'posicion' => $model->getPosicion(),
            'posicionAddr' => $model->getPosicionAddr(),
            'costeHora' => $model->getCosteHora(),
            'pesoNeto' => $model->getPesoNeto(),
            'itv' => $model->getItv(),
            'otros' => $model->getOtros(),
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\Camiones
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\Camiones');
        }
     ...
     *
     * @param Atezate\Model\Raw\Camiones $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\Camiones) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\Camiones object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\Camiones object in delete for " . get_class($this);
            }
        ...
        try {
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\Camiones', $capitalizedFk);
            ...
            $where = $dbAdapter->quoteInto($dbAdapter->quoteIdentifier('camionId') . ' = ?', $model->getCamionId());
     ...
     */
    public function save(\Atezate\Model\Raw\Camiones $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\Camiones $model, $useTransaction = true, $transactionTag = null)
    {
            ...
            if (is_null($primaryKey) || empty($primaryKey)) {
                ...
                $this->getDbTable()
                     ->update(
                         $data,
                         array(
                             $this->getDbTable()->getAdapter()->quoteIdentifier('camionId') . ' = ?' => $primaryKey
                         )
                     );
            }
            ...
            if ($recursive) {
                ...
                if ($model->getTurnosRelCamiones(null, null, true) !== null) {
                    ...
                    foreach ($turnosRelCamiones as $value) {
                        $value->setCamionesId($primaryKey)
                              ->saveRecursive(false, $transactionTag);
                    }
                }
     ...
     * @param Atezate\Model\Raw\Camiones|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\Camiones The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\Camiones();
        }
        ...
        if (is_array($data)) {
            $entry->setCamionId($data['camionId'])
                  ->setMatricula($data['matricula'])
                  ->setPrecision($data['precision'])
                  ->setCuandoPosicion($data['cuandoPosicion'])
                  ->setPosicion($data['posicion'])
                  ->setPosicionAddr($data['posicionAddr'])
                  ->setCosteHora($data['costeHora'])
        ...
        } else if ($data instanceof \Atezate\Model\Raw\Camiones) {
            $entry->setCamionId($data->getCamionId())
                  ->setMatricula($data->getMatricula())
                  ->setPrecision($data->getPrecision())
                  ->setCuandoPosicion($data->getCuandoPosicion())
                  ->setPosicion($data->getPosicion())
                  ->setPosicionAddr($data->getPosicionAddr())
                  ->setCosteHora($data->getCosteHora())
                  ->setPesoNeto($data->getPesoNeto())
                  ->setItv($data->getItv())
