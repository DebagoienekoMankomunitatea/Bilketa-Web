...
/**
 * Data Mapper implementation for Atezate\Model\Mensajes
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
class Mensajes extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\Mensajes';
     ...
     *
     * @param Atezate\Model\Mensajes $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Mensajes) {
            throw new \Exception('Unable to create array: invalid model passed to mapper', 2000);
        }

        $result = array(
            'mensajeId' => $model->getMensajeId(),
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\Mensajes
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\Mensajes');
        }
     ...
     *
     * @param Atezate\Model\Mensajes $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        ...
        try {
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitzalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\Mensajes', $capitzalizedFk);
            ...
            $where = $dbAdapter->quoteInto($dbAdapter->quoteIdentifier('mensajeId') . ' = ?', $model->getMensajeId());
     ...
     */
    public function save(\Atezate\Model\Mensajes $model)
    {
     ...
     * @return boolean If the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Mensajes $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Mensajes $model,
    ...
    )
    {
        ...
        unset($data['mensajeId']);

        try {
            if (is_null($primaryKey) || empty($primaryKey)) {
                $primaryKey = $this->getDbTable()->insert($data);
                if ($primaryKey) {
                    $model->setMensajeId($primaryKey);
                } else {
                ...
                $this->getDbTable()
                     ->update(
                         $data,
                         array(
                             $this->getDbTable()->getAdapter()->quoteIdentifier('mensajeId') .' = ?' => $primaryKey
                         )
                     );
            }
            ...
            if ($recursive) {
                if ($model->getLlamadas(null, null, true) !== null) {
                    $llamadas = $model->getLlamadas();

                    if (!is_array($llamadas)) {

                        $llamadas = array($llamadas);
                    }

                    foreach ($llamadas as $value) {
                        $value->setMensajeId($primaryKey)
     ...
     * @param Atezate\Model\Mensajes|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Mensajes The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\Mensajes();
        }
        ...
        if (is_array($data)) {
            $entry->setMensajeId($data['mensajeId'])
                  ->setMensaje($data['mensaje']);
        } else if ($data instanceof \Zend_Db_Table_Row_Abstract || $data instanceof \stdClass) {
            $entry->setMensajeId($data->{'mensajeId'})
                  ->setMensaje($data->{'mensaje'});

        } else if ($data instanceof \Atezate\Model\Mensajes) {
            $entry->setMensajeId($data->getMensajeId())
