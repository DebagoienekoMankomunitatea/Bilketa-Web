...
/**
 * Data Mapper implementation for Atezate\Model\TiposIncidencias
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
class TiposIncidencias extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\TiposIncidencias';
     ...
     *
     * @param Atezate\Model\Raw\TiposIncidencias $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\TiposIncidencias) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\TiposIncidencias object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\TiposIncidencias object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            ...
            'gravedad' => $model->getGravedad(),
            'tipo' => $model->getTipo(),
            'descripcion' => $model->getDescripcion(),
            'resolucionAutomatica' => $model->getResolucionAutomatica(),
            'plantillasEmailId' => $model->getPlantillasEmailId(),
            'plantillasEmailPrioridad' => $model->getPlantillasEmailPrioridad(),
            'plantillasSmsId' => $model->getPlantillasSmsId(),
            'plantillasSmsPrioridad' => $model->getPlantillasSmsPrioridad(),
            'plantillasTelefonoId' => $model->getPlantillasTelefonoId(),
            'plantillasTelefonoPrioridad' => $model->getPlantillasTelefonoPrioridad(),
        );
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\TiposIncidencias
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\TiposIncidencias');
        }
     ...
     *
     * @param Atezate\Model\Raw\TiposIncidencias $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\TiposIncidencias) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\TiposIncidencias object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\TiposIncidencias object in delete for " . get_class($this);
            }
        ...
        try {
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\TiposIncidencias', $capitalizedFk);
            ...
            $where = $dbAdapter->quoteInto($dbAdapter->quoteIdentifier('tipoId') . ' = ?', $model->getTipoId());
     ...
     */
    public function save(\Atezate\Model\Raw\TiposIncidencias $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\TiposIncidencias $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Raw\TiposIncidencias $model,
            ...
            if (is_null($primaryKey) || empty($primaryKey)) {
                ...
                $this->getDbTable()
                     ->update(
                         $data,
                         array(
                             $this->getDbTable()->getAdapter()->quoteIdentifier('tipoId') . ' = ?' => $primaryKey
                         )
                     );
            }
     ...
     * @param Atezate\Model\Raw\TiposIncidencias|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\TiposIncidencias The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\TiposIncidencias();
        }
        ...
        if (is_array($data)) {
            $entry->setTipoId($data['tipoId'])
                  ...
                  ->setResolucionAutomatica($data['resolucionAutomatica'])
                  ->setPlantillasEmailId($data['plantillasEmailId'])
                  ->setPlantillasEmailPrioridad($data['plantillasEmailPrioridad'])
                  ->setPlantillasSmsId($data['plantillasSmsId'])
                  ->setPlantillasSmsPrioridad($data['plantillasSmsPrioridad'])
                  ->setPlantillasTelefonoId($data['plantillasTelefonoId'])
                  ->setPlantillasTelefonoPrioridad($data['plantillasTelefonoPrioridad']);
        ...
        } else if ($data instanceof \Atezate\Model\Raw\TiposIncidencias) {
            $entry->setTipoId($data->getTipoId())
                  ->setGravedad($data->getGravedad())
                  ->setTipo($data->getTipo())
                  ->setDescripcion($data->getDescripcion())
                  ->setResolucionAutomatica($data->getResolucionAutomatica())
                  ->setPlantillasEmailId($data->getPlantillasEmailId())
                  ->setPlantillasEmailPrioridad($data->getPlantillasEmailPrioridad())
                  ->setPlantillasSmsId($data->getPlantillasSmsId())
                  ->setPlantillasSmsPrioridad($data->getPlantillasSmsPrioridad())
                  ->setPlantillasTelefonoId($data->getPlantillasTelefonoId())
                  ->setPlantillasTelefonoPrioridad($data->getPlantillasTelefonoPrioridad());
