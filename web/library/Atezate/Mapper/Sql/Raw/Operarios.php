...
/**
 * Data Mapper implementation for Atezate\Model\Operarios
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
class Operarios extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\Operarios';
     ...
     *
     * @param Atezate\Model\Raw\Operarios $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\Operarios) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\Operarios object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\Operarios object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            'operarioId' => $model->getOperarioId(),
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\Operarios
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\Operarios');
        }
     ...
     *
     * @param Atezate\Model\Raw\Operarios $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\Operarios) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\Operarios object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\Operarios object in delete for " . get_class($this);
            }
        ...
        try {
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\Operarios', $capitalizedFk);
            ...
            $where = $dbAdapter->quoteInto($dbAdapter->quoteIdentifier('operarioId') . ' = ?', $model->getOperarioId());
     ...
     */
    public function save(\Atezate\Model\Raw\Operarios $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\Operarios $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Raw\Operarios $model,
    ...
    )
    {
        ...
        $primaryKey = $model->getOperarioId();
        $success = true;
        ...
        try {
            if (is_null($primaryKey) || empty($primaryKey)) {
                ...
                if ($primaryKey) {
                    $model->setOperarioId($primaryKey);
                } else {
                ...
                $this->getDbTable()
                     ->update(
                         $data,
                         array(
                             $this->getDbTable()->getAdapter()->quoteIdentifier('operarioId') . ' = ?' => $primaryKey
                         )
                     );
            }
            ...
            if ($recursive) {
                if ($model->getRevision(null, null, true) !== null) {
                    ...
                    foreach ($revision as $value) {
                        $value->setOperadorId($primaryKey)
                ...
                if ($model->getTurnosCamionesRelOperarios(null, null, true) !== null) {
                    ...
                    foreach ($turnosCamionesRelOperarios as $value) {
                        $value->setOperarioId($primaryKey)
                              ->saveRecursive(false, $transactionTag);
                    }
                }
     ...
     * @param Atezate\Model\Raw\Operarios|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\Operarios The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\Operarios();
        }
        ...
        if (is_array($data)) {
            $entry->setOperarioId($data['operarioId'])
        ...
        } else if ($data instanceof \Atezate\Model\Raw\Operarios) {
            $entry->setOperarioId($data->getOperarioId())
                  ->setNombre($data->getNombre())
                  ->setRolId($data->getRolId());
