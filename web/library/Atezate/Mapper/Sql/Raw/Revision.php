...
/**
 * Data Mapper implementation for Atezate\Model\Revision
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
class Revision extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\Revision';
     ...
     *
     * @param Atezate\Model\Raw\Revision $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\Revision) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\Revision object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\Revision object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            'revisionId' => $model->getRevisionId(),
            'compostadoresId' => $model->getCompostadoresId(),
            'fechaHora' => $model->getFechaHora(),
            'operadorId' => $model->getOperadorId(),
            'revisionTipoId' => $model->getRevisionTipoId(),
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\Revision
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\Revision');
        }
     ...
     *
     * @param Atezate\Model\Raw\Revision $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\Revision) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\Revision object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\Revision object in delete for " . get_class($this);
            }
        ...
        try {
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\Revision', $capitalizedFk);
            ...
            $where = $dbAdapter->quoteInto($dbAdapter->quoteIdentifier('revisionId') . ' = ?', $model->getRevisionId());
     ...
     */
    public function save(\Atezate\Model\Raw\Revision $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\Revision $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Raw\Revision $model,
    ...
    )
    {
        ...
        $primaryKey = $model->getRevisionId();
        $success = true;
        ...
        try {
            if (is_null($primaryKey) || empty($primaryKey)) {
                ...
                if ($primaryKey) {
                    $model->setRevisionId($primaryKey);
                } else {
                ...
                $this->getDbTable()
                     ->update(
                         $data,
                         array(
                             $this->getDbTable()->getAdapter()->quoteIdentifier('revisionId') . ' = ?' => $primaryKey
                         )
                     );
            }
     ...
     * @param Atezate\Model\Raw\Revision|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\Revision The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\Revision();
        }
        ...
        } else if ($data instanceof \Atezate\Model\Raw\Revision) {
            $entry->setRevisionId($data->getRevisionId())
                  ->setCompostadoresId($data->getCompostadoresId())
                  ->setFechaHora($data->getFechaHora())
                  ->setOperadorId($data->getOperadorId())
                  ->setRevisionTipoId($data->getRevisionTipoId())
