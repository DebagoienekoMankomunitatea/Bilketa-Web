...
/**
 * Data Mapper implementation for Atezate\Model\Ts
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
class Ts extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\Ts';
     ...
     *
     * @param Atezate\Model\Ts $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Ts) {
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\Ts
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\Ts');
        }
     ...
     *
     * @param Atezate\Model\Ts $model The model to delete
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitzalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\Ts', $capitzalizedFk);
     ...
     */
    public function save(\Atezate\Model\Ts $model)
    {
     ...
     * @return boolean If the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Ts $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Ts $model,
     ...
     * @param Atezate\Model\Ts|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Ts The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\Ts();
        }
        ...
        } else if ($data instanceof \Atezate\Model\Ts) {
