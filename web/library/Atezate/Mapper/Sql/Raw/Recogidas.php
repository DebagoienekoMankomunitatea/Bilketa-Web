...
/**
 * Data Mapper implementation for Atezate\Model\Recogidas
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
class Recogidas extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\Recogidas';
     ...
     *
     * @param Atezate\Model\Raw\Recogidas $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\Recogidas) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\Recogidas object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\Recogidas object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            'id' => $model->getId(),
            'recogidaTipoId' => $model->getRecogidaTipoId(),
            ...
            'gradoLlenado' => $model->getGradoLlenado(),
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\Recogidas
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\Recogidas');
        }
     ...
     *
     * @param Atezate\Model\Raw\Recogidas $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\Recogidas) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\Recogidas object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\Recogidas object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\Recogidas', $capitalizedFk);
     ...
     */
    public function save(\Atezate\Model\Raw\Recogidas $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\Recogidas $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Raw\Recogidas $model,
     ...
     * @param Atezate\Model\Raw\Recogidas|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\Recogidas The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\Recogidas();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ->setRecogidaTipoId($data['recogidaTipoId'])
                  ...
                  ->setGradoLlenado($data['gradoLlenado'])
        ...
        } else if ($data instanceof \Atezate\Model\Raw\Recogidas) {
            $entry->setId($data->getId())
                  ->setRecogidaTipoId($data->getRecogidaTipoId())
                  ->setCuboId($data->getCuboId())
                  ->setTipos($data->getTipos())
                  ...
                  ->setGradoLlenado($data->getGradoLlenado())
