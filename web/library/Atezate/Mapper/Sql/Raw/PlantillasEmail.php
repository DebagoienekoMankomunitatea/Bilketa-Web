...
/**
 * Data Mapper implementation for Atezate\Model\PlantillasEmail
 *
 * @package Atezate\Mapper\Sql
 * @subpackage Raw
 * @author Victor Vargas
...
class PlantillasEmail extends MapperAbstract
{
    protected $_modelName = 'Atezate\\Model\\PlantillasEmail';
     ...
     *
     * @param Atezate\Model\Raw\PlantillasEmail $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Atezate\Model\Raw\PlantillasEmail) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Atezate\Model\Raw\PlantillasEmail object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\Model\\PlantillasEmail object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            'id' => $model->getId(),
            'asunto_es' => $model->getAsuntoEs(),
            'asunto_eu' => $model->getAsuntoEu(),
            'mensaje_es' => $model->getMensajeEs(),
            'mensaje_eu' => $model->getMensajeEu(),
        );
     ...
     *
     * @return Atezate\\Mapper\\Sql\\DbTable\\PlantillasEmail
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Atezate\\Mapper\\Sql\\DbTable\\PlantillasEmail');
        }
     ...
     *
     * @param Atezate\Model\Raw\PlantillasEmail $model The model to delete
    ...
    public function delete(\Atezate\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Atezate\Model\Raw\PlantillasEmail) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Atezate\\Model\\PlantillasEmail object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Atezate\\Model\\PlantillasEmail object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Atezate\\Mapper\\Sql\\DbTable\\PlantillasEmail', $capitalizedFk);
     ...
     */
    public function save(\Atezate\Model\Raw\PlantillasEmail $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Atezate\Model\Raw\PlantillasEmail $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Atezate\Model\Raw\PlantillasEmail $model,
    ...
    )
    {
        ...
        try {
            ...
            if ($recursive) {
                if ($model->getTiposIncidencias(null, null, true) !== null) {
                    $tiposIncidencias = $model->getTiposIncidencias();

                    if (!is_array($tiposIncidencias)) {

                        $tiposIncidencias = array($tiposIncidencias);
                    }

                    foreach ($tiposIncidencias as $value) {
                        $value->setPlantillasEmailId($primaryKey)
     ...
     * @param Atezate\Model\Raw\PlantillasEmail|null $entry The object to load the data into, or null to have one created
     * @return Atezate\Model\Raw\PlantillasEmail The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Atezate\Model\PlantillasEmail();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ->setAsuntoEs($data['asunto_es'])
                  ->setAsuntoEu($data['asunto_eu'])
                  ->setMensajeEs($data['mensaje_es'])
                  ->setMensajeEu($data['mensaje_eu']);
        ...
        } else if ($data instanceof \Atezate\Model\Raw\PlantillasEmail) {
            $entry->setId($data->getId())
                  ->setAsuntoEs($data->getAsuntoEs())
                  ->setAsuntoEu($data->getAsuntoEu())
                  ->setMensajeEs($data->getMensajeEs())
                  ->setMensajeEu($data->getMensajeEu());
